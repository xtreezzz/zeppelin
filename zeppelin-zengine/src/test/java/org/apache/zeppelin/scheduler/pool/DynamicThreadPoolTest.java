...
public class DynamicThreadPoolTest {
  ...
  /**
   * MAX_POOL_SIZE - maximum pool size in test in [1; MAX_POOL_SIZE].
   * COUNT_OF_JOBS - count of jobs on each test iteration.
  ...
  /**
   * Creates 2 jobs {@link SimpleTask} each is sleeps for 1 second.
   *  if threadCount = 1 there is not enough free threads to run jobs concurrently,
   *      therefore execution lasts 2 seconds;
   *  if threadCount = 2 jobs will be executed concurrently, therefore
   *      execution lasts for 1 second.
   *
  ...
  public void testConcurrentExecution() throws SchedulerConfigException {
    ...
    for (int threadCount = 1; threadCount <= MAX_POOL_SIZE; ++threadCount) {
      ...
      Assert.assertEquals("Thread count update failed", threadCount, threadPool.getPoolSize());
      stopwatch.start();
