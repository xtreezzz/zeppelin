...
public class NotebookService {
  ...
  public void updateNote(String noteId,
    ...
    LOGGER.debug("[CRON]: update note with id={}, cronEenabled? - {}", noteId,
    ...
    if (!(Boolean) note.getConfig().get("isZeppelinNotebookCronEnable")) {
      if (config.get("cron") != null) {
        LOGGER.debug("[CRON] cron is not enabled - remove cron from note[id={}]", noteId);
        config.remove("cron");
      }
    }
    boolean cronUpdated = isCronUpdated(config, note.getConfig());
    LOGGER.debug("[CRON] is cron updated? - {}\n\tNew config - {}\n\tOld - {}", cronUpdated,
        config, note.getConfig());
    note.setName(name);
    note.setConfig(config);
    if (cronUpdated) {
      LOGGER.debug("[CRON] Refresing cron for note[id={}]", note.getId());
