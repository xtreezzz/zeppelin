...
public class UniverseUtil {
  ...
  public UniverseQuery convertQuery(String text, UniverseClient client, String token)
    ...
    for (int i = 0; i < array.length; i++) {
      ...
      if (!selectPart && !wherePart && pathClosed && doubleQuoteClosed && singleQuoteClosed
      ...
      if (createPart) {
        if (pathClosed && doubleQuoteClosed && singleQuoteClosed && buf.toString().toLowerCase()
            .endsWith("as")) {
          create.append(c);
          createPart = false;
    ...
    return new UniverseQuery(createTableName, select.toString().trim(), where.toString().trim(),
        universeInfo);
